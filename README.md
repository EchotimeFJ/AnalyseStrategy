...
在研究助手页面右上角点击“AI 配置”，输入网站管理员密码后，可选择已保存的配置，或修改服务商、模型、API 基础地址以创建另一套配置。每个“服务商＋接口地址＋模型”组合分别加密保存 Key；只有同一组合才能留空复用已保存的 Key，新组合必须填写。测试连接不会切换全站配置，点击“保存并启用”才生效，原有配置仍保留。现有用量限制保持不变。服务器环境变量覆盖配置时，弹窗会提示需要先移除对应覆盖项。
