...
配置弹窗中的服务商按钮优先打开已保存的模型，并保留本次弹窗内各配置尚未保存的输入。每个“服务商＋接口地址＋模型”组合分别加密保存 Key；新组合必须填写自己的 Key。“保存配置”只保存当前编辑项，不切换模型；编辑正在使用的配置时，其 Key 更新会立即生效。“使用已保存配置”用于切换全站模型。关闭弹窗会清除未保存输入，现有用量限制保持不变。服务器环境变量覆盖配置时，需先移除对应覆盖项。
...
对话与后台复核共用并发名额和每日额度。请求前按输入 UTF-8 字节数和输出上限保守预留；供应商返回实际用量后据实结算，未返回用量的超时、断连等请求保留预留值。该额度用于限制调用，不代表供应商账单；实际费用以服务商记录为准。
