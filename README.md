...
- 索引管理：刷新本地 Markdown 索引，也支持对 Strategy 仓库执行 `git pull --ff-only` 后重建索引，并展示本次新增、修改、删除的日报日期。

## 交互规则

- 搜索、标的分析等查询页首次进入时不填默认标的。
...
- `npm test`：运行解析、Git 更新、排序、评级搜索和报告变更 diff 回归测试。
